...
const likes = async (idx) => {
    const data = await api.likes(idx);

    if (data) {
        const target = feedList.find((feed) => feed.idx === idx);
        if (target) {
            target.isLikes = !target.isLikes;
    ...
    <main class="main-container">
        <section class="content-container">
            <div class="content">
                ...
                <div v-for="feed in feedList" :key="feedList.idx" class="posts">
                    <article class="post">
                        <div class="post__header">
                            <div class="post__profile">
                                ...
                                <a target="_blank" class="post__user">{{
                                    feed.writer }}</a>
                            </div>
                        ...
                        <div class="post__content">
                            <div class="post__medias">
                                <Carousel :wrap-around="true" :items-to-show="1">
                                    <Slide v-for="(imageUrl, index) in feed.imageUrls" :key="index">
                                        <img class="post__media" :src="imageUrl" />
                                    </Slide>
                        ...
                        <div class="post__footer">
                            <div class="post__buttons">
                                <button class="post__button" @click="likes(feed.idx)">
                                    <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"
                                        :style="{ '--text-dark': feed.isLikes ? 'red' : 'black' }">
                                        <path
